<template>
  <div class="flex lg:mx-45 mx-12 justify-center items-center pb-12 lg:px-36">
    <div class="p-6 w-full flex flex-col gap-5 bg-dark rounded-md border border-lightmode text-lightmode">
      ...
      <div class="flex gap-12 flex-col">
            ...
            <label for="" class="relative flex items-center gap-5">
              <div class="">
                <input v-model="payload.komentar" type="text"
                  class="bg-secondary z-10 text-white placeholder:text-lightmode w-[15rem] lg:w-[54rem] border border-lightmode rounded-full py-4 pl-8"
        ...
        <div v-for="item in komentar" class="flex gap-5 items-center border-t-2 border-t-lightmode">
          ...
          <div class="flex flex-col relative">
            <div class="flex gap-4 items-center">
              ...
              <h3 class="text-lightmode bg-secondary p-2 rounded-md">
                {{ item.user?.kelas }}
              </h3>
            </div>
            <input disabled type="text"
              class="bg-tertiary text-white placeholder:text-lightmode lg:w-96 border-none p-0 py-4"
              :value="item.komentar" />
            <div class="flex gap-2">
              <button @click="like(item.id)" class="text-lightmode w-6 flex">
                <span v-if="liked == false">
                  <HeartIcon class="w-6 h-6 text-lightmode"></HeartIcon>
                </span>
                <span v-else>
