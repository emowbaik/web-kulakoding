  ...
  <div class="w-full h-full overflow-hidden">
    ...
    <div class="flex flex-col lg:flex-row justify-between">
      <div class="text-center mx-auto lg:w-1/2">
        ...
        <p v-show="showAbout" class="hidden lg:block text-base font-medium text-center mx-5 max-w-[800px]">
          {{ project.deskripsi }}
        </p>
      ...
      <div class="hidden lg:block text-center mt-20">
        <div class="flex flex-col gap-5 bg-slate-300 dark:bg-dark shadow rounded-md mx-5 py-10">
          <img :src="`https://api.dicebear.com/7.x/initials/svg?seed=${project.user?.username}`" alt="foto_project"
            class="w-[70%] h-auto lg:w-[50%] lg:h-[50%] rounded-full mx-auto bg-center bg-cover bg-no-repeat p-2" />
          <p class="font-bold text-lg dark:text-light">
            {{ project.user?.username }}<br />{{ project.user?.kelas }}
          </p>
        </div>
      ...
      <div class="lg:hidden text-center">
        <span class="flex mx-5 gap-5 items-center">
          <div class="bg-slate-300 dark:bg-dark shadow rounded-lg">
            <img :src="`https://api.dicebear.com/7.x/initials/svg?seed=${project.user?.username}`" alt="foto_project"
      ...
      <!-- End Profile -->
      <p v-show="showAbout" class="lg:hidden text-base font-medium text-center mx-5 break-words max-w-[800px]">
        {{ project.deskripsi }}
      </p>
